...
import 'custom_input_date_picker_form_field.dart' as custom_input_date_picker_form_field;
...
const Size _calendarPortraitDialogSizeM2 = Size(330.0, 408.0); // Custom change
const Size _calendarPortraitDialogSizeM3 = Size(328.0, 402.0); // Custom change
...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  final _globalTextWidgetDatePickerKey = GlobalKey();    // Custom new
  final _globalControlWidgetDatePickerKey = GlobalKey(); // Custom new
  ...
  Widget build(BuildContext context) {
    ...
    headlineStyle = headlineStyle?.copyWith(color: theme.colorScheme.brightness == Brightness.light ? theme.colorScheme.primary : theme.colorScheme.onSurface);
    ...
    final Widget actions = Container(
      ...
      child: OverflowBar(
        spacing: 8,
        children: <Widget>[
          ElevatedButton(                                                                           // Custom change
            style: ElevatedButton.styleFrom(                                                        // Custom change
              ...
              side: BorderSide(                                                                     // Custom change
                color: Theme.of(context).colorScheme.primary,                                       // Custom change
              ),                                                                                    // Custom change
            ),                                                                                      // Custom change
            onPressed: _handleCancel,                                                               // Custom change
            child: Text(                                                                            // Custom change
              context.translate.cancel.alternative(), // Custom change
              style: const TextStyle(                                                               // Custom change
                fontWeight: FontWeight.normal,                                                      // Custom change
              ),                                                                                    // Custom change
            ),                                                                                      // Custom change
          ),                                                                                        // Custom change
          Padding(                                                                                  // Custom change
            padding: const EdgeInsets.only(                                                         // Custom change
              right: 15,                                                                            // Custom change
            ),                                                                                      // Custom change
            child: ElevatedButton(                                                                  // Custom change
    ...
    custom_calendar_date_picker.CalendarDatePicker calendarDatePicker() { // Custom change
      return custom_calendar_date_picker.CalendarDatePicker(              // Custom change
        ...
        child: Container(
          ...
          child: Shortcuts(
            ...
            child: Column(
              children: <Widget>[
                const Spacer(),
                CustomDateField(                                // Custom new
                  textWidget: Container(                        // Custom new
                    key: _globalTextWidgetDatePickerKey,        // Custom new
                    padding: const EdgeInsets.only(             // Custom new
                      bottom: 10                                // Custom new
                    ),                                          // Custom new
                    child: Text(                                // Custom new
                      localizations.dateInputLabel,             // Custom new
                      style: getCustomTextWidgetStyle(),        // Custom new
                    ),                                          // Custom new
                  ),                                            // Custom new
                  controlWidget: custom_input_date_picker_form_field.InputDatePickerFormField( // Custom new
                    key: _globalControlWidgetDatePickerKey,                                    // Custom new
                    initialDate: _selectedDate.value,
                    firstDate: widget.firstDate,
                    lastDate: widget.lastDate,
    ...
    switch (_entryMode.value) {
      case DatePickerEntryMode.calendar:
        ...
        entryModeButton = orientation == Orientation.portrait ? IconButton( // Custom change
...
class _DatePickerHeader extends StatelessWidget {
  ...
  static const double _datePickerHeaderPortraitHeight = 90.0; // Custom change
  ...
  Widget build(BuildContext context) {
    ...
    final Text title = Text(
      ...
      style: TextStyle(               // Custom change
        fontSize: 24,                 // Custom change
        fontWeight: FontWeight.bold,  // Custom change
        color: foregroundColor,       // Custom change
      ),                              // Custom change
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  help,
                  const Flexible(child: SizedBox(height: 8)), // Custom change
                  Row(
                    children: <Widget>[
...
const double _maxCalendarWidthLandscape = double.maxFinite;                             // Custom change
...
class _MonthItemState extends State<_MonthItem> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: <Widget>[
        Container(
          padding: EdgeInsets.symmetric(horizontal: context.screenWidth*0.065),       // Custom change
